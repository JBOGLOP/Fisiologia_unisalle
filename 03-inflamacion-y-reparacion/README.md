# Encuentro 3 — Inflamación y reparación de tejidos

**6 de agosto de 2026** · Clase · Fisiopatología General (`01008430`) · Optometría · Universidad de La Salle

| Estado | Material | Duración |
|---|---|---|
| 🔜 **Próximo encuentro** | ✅ Publicado | 2 horas |

---

## De qué trata

Cierra el bloque de fundamentos. La lesión celular explicó qué le pasa a la célula agredida; esta sesión explica cómo responde el tejido entero. Es el mecanismo que sostiene el resto del semestre: la fiebre, el dolor y el edema son manifestaciones suyas, y por eso se ve antes que ellas.

## Material de clase

### ▶ [Abrir la clase interactiva](https://jboglop.github.io/Fisiologia_unisalle/03-inflamacion-y-reparacion/clase.html)

Se abre directamente en el navegador. También puedes [descargar el archivo](clase.html) para usarla sin conexión.

### 👥 [Taller en grupos de la sesión](https://jboglop.github.io/Fisiologia_unisalle/03-inflamacion-y-reparacion/actividad.html)

La segunda hora de la clase. Tres grupos, tres casos clínicos oculares y una cadena fisiopatológica que construir. Las cinco preguntas de cada caso son del nivel del primer parcial.

## Contenidos

- Inflamación aguda: concepto, propósito y desenlaces posibles
- Mediadores químicos de la respuesta inflamatoria
- Cambios vasculares: calibre, flujo y permeabilidad
- Sucesos celulares: marginación, rodamiento, adhesión y transmigración
- Patrones morfológicos de la inflamación aguda
- Reparación: regeneración celular frente a sustitución por tejido conjuntivo
- Fibrosis y cicatrización; aspectos patológicos de la cicatrización
- Aplicación ocular: cicatrización corneal y por qué la transparencia condiciona el desenlace

## Lectura previa

- Uribe Olivares, cap. 19

## Trabajo independiente

- Lectura del capítulo 19 del texto guía
- Esquema propio de la secuencia vascular y celular de la inflamación aguda
- Repaso integral de los capítulos 1 a 6 y 19 para el primer parcial

## Resultados de aprendizaje que se movilizan

- **RA17** — Explica la función de los sistemas del cuerpo humano, la patogénesis de las enfermedades sistémicas y sus efectos sobre la salud visual y ocular
- **RA18** — Analiza los cambios del funcionamiento normal de un tejido, órgano o sistema para diferenciar lo normal de lo patológico

---

← [Encuentro 2: Inducción: síndrome y mecanismos de lesión](../02-sindrome-y-lesion/) · [Índice del curso](../) · [Encuentro 4: Primer parcial](../04-primer-parcial/) →
